...
static void NVGFreeTextFrame(NVGTextFrame frame)
{
  if (frame.count) {
...
- (void)setTextFrame:(NVGTextFrame)frame
{
  if (frame.lines != _textFrame.lines) {
    NVGFreeTextFrame(_textFrame);
  }
  [self invalidate];
  _textFrame = frame;
}
...
{
  NVGTextFrame frame = self.textFrame;
...
//    NVGCGFloatArray dash = self.strokeDash;
//    if (dash.count) {
...
- (void)renderLineTo:(NVGcontext*)context atIndex:(int)index
{
//  NVGTextFrame frame = self.textFrame;
//  CGFloat shift;
